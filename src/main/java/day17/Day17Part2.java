...
public class Day17Part2 {
    ...
    record Bounds(Coordinate min, Coordinate max) {
        static final Bounds MIN = new Bounds(new Coordinate(0, 0, 0, 0), new Coordinate(0, 0, 0, 0));

        Bounds combine(Bounds b) {
            return new Bounds(
                    new Coordinate(Math.min(min.x, b.min.x), Math.min(min.y, b.min.y), Math.min(min.z, b.min.z), Math.min(min.w, b.min.w)),
                    new Coordinate(Math.max(max.x, b.max.x), Math.max(max.y, b.max.y), Math.max(max.z, b.max.z), Math.max(max.w, b.max.w)));
        }

        Bounds extend(Coordinate coordinate) {
            return new Bounds(
                    new Coordinate(Math.min(min.x, coordinate.x), Math.min(min.y, coordinate.y), Math.min(min.z, coordinate.z), Math.min(min.w, coordinate.w)),
                    new Coordinate(Math.max(max.x, coordinate.x), Math.max(max.y, coordinate.y), Math.max(max.z, coordinate.z), Math.max(max.w, coordinate.w)));
        }
    }
    ...
    record State(Set<Coordinate> active) {
        ...
        void print() {
            ...
            for (var w = extents.min.w; w <= extents.max.w; w++) {
                for (var z = extents.min.z; z <= extents.max.z; z++) {
                    ...
                    for (var y = extents.min.y; y <= extents.max.y; y++) {
                        for (var x = extents.min.x; x <= extents.max.x; x++) {
                            System.out.print(active.contains(new Coordinate(x, y, z, w)) ? '#' : '.');
                        }
                        System.out.println();
                    }
                    System.out.println();
            ...
            for (int x = extents.min.x - 1; x <= extents.max.x + 1; x++) {
                for (int y = extents.min.y - 1; y <= extents.max.y + 1; y++) {
                    for (int z = extents.min.z - 1; z <= extents.max.z + 1; z++) {
                        for (int w = extents.min.w - 1; w <= extents.max.w + 1; w++) {
                            var current = new Coordinate(x, y, z, w);
        ...
        static State load(String file) throws IOException, URISyntaxException {
            var lines = Files.readAllLines(Paths.get(Day17Part2.class.getResource("/day17/" + file).toURI()));
            ...
            for (var y = 0; y < lines.size(); y++) {
                ...
                for (var x = 0; x < chars.length; x++) {
                    if (chars[x] == '#') {
                        active.add(new Coordinate(x, y, 0, 0));
                    }
                }
            }
            ...
            return IntStream.rangeClosed(coordinate.x - 1, coordinate.x + 1)
                    ...
                    .flatMap(xn -> IntStream.rangeClosed(coordinate.y - 1, coordinate.y + 1)
                            ...
                            .flatMap(yn -> IntStream.rangeClosed(coordinate.z - 1, coordinate.z + 1)
                                    .boxed()
                                    .flatMap(zn -> IntStream.rangeClosed(coordinate.w - 1, coordinate.w + 1)
                                            .mapToObj(wn -> new Coordinate(xn, yn, zn, wn))
